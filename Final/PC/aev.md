...
## Message Event
<img align="left" src="images/Events/Message Event.png" width = 200 height = 200></img> A Messsage event is used to display text on screen. When this event is triggered; the game will check the [[sound effect](#sound-effect-number) type](#sound-effect-type) to determine which soundbank to look use, if any at all, for a sound that is to be played when the message is triggered. If one is assigned, it will grab that [sound effect](#sound-effect-number) from the previously designated sound table.  (This is hard coded, and unable to be edited without a mod to the binary) Next the game will adjust the [camera's focus](#camera-number) to a specific CAM index. Finally after setting everything up the game will display a specific index from the desired MDT file. <br><br> <br><br> <br><br>
